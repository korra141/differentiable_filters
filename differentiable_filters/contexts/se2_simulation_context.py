...
from differentiable_filter.se2_group import SE2Group


class SE2_Context(base.BaseContext):
    ...
    def neg_log_likelihood(self, input_output_tuple, grid_size):
        ...
        ln_z_ = tf.cast(tf.math.real(
            ...
            dtype=tf.float32)  # [batch_size, trajectory_length, 1]
        k_values = tf.range(0, self.grid_size, 1, dtype=tf.dtypes.float32) - self.zeroth_freq_index
    ...
    def create_and_save_datasets(self, starting_positions, trajectory_length, control_step, train_size,
        ...
        self.beacons = np.array(
            [[0, 0.1],
             ...
             [0, -0.05],
             [0, -0.1]])
        self.beacon_idx = [0,1,2,3,4,5]
        for i in range(n_samples):
            position = SE2Group.from_parameters(x=starting_positions[i, 0], y=starting_positions[i, 1],theta=starting_positions[i, 2])
            for j in range(trajectory_length):
                position = position @ custom_step
