...
class S1ToyContext(base.BaseContext):
    ...
    # process model
    ###########################################################################
    def run_process_model(self, control, old_state=None, training=False):
        """
    ...
    def get_loss(self, data, label, prediction):
        ...
        # pdb.set_trace()
        # abs_mean_pose_posterior = tf.math.minimum(2*math.pi*tf.ones_like(pose) - tf.math.abs(mean_pose_posterior - pose), tf.math.abs(mean_pose_posterior - pose))
        # abs_mean_pose_posterior_s1 = tf.math.minimum(2*math.pi*tf.ones_like(pose)- tf.math.abs(mean_pose_posterior_s1 - pose), tf.math.abs(mean_pose_posterior_s1 - pose))
        # abs_mean_pose_pred = tf.math.minimum(2*math.pi*tf.ones_like(pose) - tf.math.abs(mean_pose_pred - pose), tf.math.abs(mean_pose_pred - pose))
        # abs_mean_pose_pred_s1 = tf.math.minimum(2*math.pi*tf.ones_like(pose) - tf.math.abs(mean_pose_pred_s1 - pose), tf.math.abs(mean_pose_pred_s1 - pose))
        # abs_mean_obs = tf.math.minimum(2*math.pi*tf.ones_like(pose) - tf.math.abs(mean_obs - observation), tf.math.abs(mean_obs - observation))
        # abs_mean_obs_s1 = tf.math.minimum(2*math.pi*tf.ones_like(pose) - tf.math.abs(mean_obs_s1 - observation), tf.math.abs(mean_obs_s1 - observation))
    ...
    def compute_mode_(self, energy_samples):
        ...
        maximum = tf.expand_dims(tf.math.reduce_max(energy_samples, axis=2), 2)
        moments = tf.signal.rfft(tf.exp(energy_samples - maximum))
        normalized_moment_0 = moments[:, :, 0] / (math.pi * self.grid_size * math.pi / 62)
        ln_z_ = tf.expand_dims(tf.math.real(tf.math.log(normalized_moment_0)), 2) + maximum
        dim = energy_samples.shape[1]
        tensor_start = tf.constant(0, dtype=tf.float64)
        tensor_stop = tf.constant(2 * math.pi, dtype=tf.float64)
        poses = tf.broadcast_to(tf.linspace(tensor_start, tensor_stop, self.grid_size + 1)[:-1], [1, 1, self.grid_size])
        ...
        # mean = tf.expand_dims(tf.cast(tf.math.real(moments[:, :, 1] / normalized_moment_0), dtype=tf.float64), 2)
        return poses_mode
    ...
    def neg_log_likelihood(self, input_output_tuple, grid_size):
        ...
        ln_z_ = tf.cast(tf.math.real(
            tf.expand_dims(tf.math.log(moments[..., 0] / (math.pi * grid_size * math.pi / 62)), -1) + maximum),
        ...
        # pdb.set_trace()
        exponential_term = tf.math.exp(tf.constant(1j, dtype=tf.complex64) * tf.cast(k_values * value,
                                                                                     dtype=tf.complex64))  # [batch_size, trajectory_length, grid_size,1]
        inverse_transform = tf.math.real(tf.math.reduce_sum(tf.expand_dims(eta, -1) * exponential_term,
                                                            axis=2))  # [batch_size, trajectory_length,  1]
