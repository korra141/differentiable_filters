...
class S1(HarmonicExponentialDistribution, metaclass=ABCMeta):
    ...
    def compute_eta(self) -> None:
        ...
        """
        # Compute energy
        # np.abs here because near zero beliefs can some times become
        # less than zero after T1FFT.sythesize().
        # And + 1e-9 because even 0 causes problems with log.
        # energy = np.log(self.prob + 1e-30)
    ...
    def compute_moments_lnz(self, eta: np.ndarray, update: bool = True) -> Tuple[np.ndarray, float]:
        ...
        maximum = np.max(negative_energy,axis=-1)[..., np.newaxis]
...
class S1Gaussian(S1):
    ...
    def __init__(self, mu_theta: float = 0.0, cov: float = 1.0, **kwargs):
        ...
        self.mu = self.theta_to_2D(self.mu_theta)
        self.cov = cov
        self.from_samples()
    ...
    def compute_energy(self, theta):
       ...
       # pdb.set_trace()
        angle = np.arccos(np.einsum('pnk,pmk->pn',x,self.mu))  # r = 1 for both so no denominator needed
